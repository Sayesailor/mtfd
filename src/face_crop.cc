...
bool VIPL::FaceCrop(const VIPLImageData &src_img, VIPLImageData &dst_img,
                    const VIPLPoint *points, int num,
                    const VIPLPoint *mean_shape, int mean_shape_size,
                    CROP_METHOD method, VIPLPoint *final_points,
                    int final_size) {
    if (final_size < 0) final_size = mean_shape_size;
    if (dst_img.width != final_size || dst_img.height != final_size ||
    ...
    bool success = face_crop_core(
        ...
        dst_img.data, mean_shape_size, mean_shape_size, crop_points.get(), num,
        ...
        (final_size - mean_shape_size) / 2,
        (final_size - mean_shape_size) - (final_size - mean_shape_size) / 2,
        final_points ? crop_final_points.get() : nullptr, type);
