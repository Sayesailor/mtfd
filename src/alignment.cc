...
static bool transformation_maker(int crop_width, int crop_height,
    ...
    for (int i = 0; i < points_num; ++i) {
        ...
        std_points[i * 2 + 1] =
            mean_shape[i * 2 + 1] * crop_height / mean_shape_height;
    }
...
#else
        if (sum_xx_yy <= __FLT_EPSILON__) return false;
#endif
...
#else
        if (!(r > __FLT_EPSILON__ || r < -__FLT_EPSILON__)) return false;
#endif
...
static void sampling(const uint8_t *image_data, int image_width,
                     int image_height, int image_channels, double scale,
    ...
    if (type == LINEAR) {
        ...
        if (ux >= 0 && ux < image_height - 1 && uy >= 0 &&
            ...
            for (int c = 0; c < image_channels; ++c) {
                ...
                ans = (1 - cof_y) * image_data[offset] +
                      cof_y * image_data[offset + image_channels];
                ans =
                    (1 - cof_x) * ans +
                    cof_x *
                        ((1 - cof_y) *
                             image_data[offset + image_width * image_channels] +
                         cof_y *
                             image_data[offset + image_width * image_channels +
                                        image_channels]);
                pixel[c] = static_cast<uint8_t>(
                    std::max<double>(0.0f, std::min<double>(255.0f, ans)));
            }
        } else {
            ...
            for (int c = 0; c < image_channels; ++c) {
                ...
                for (size_t i = 0; i < lx; ++i) {
                    ...
                    for (size_t j = 0; j < ly; ++j) {
                        val += image_data[offset +
                                          indices_y[j] * image_channels + c] *
                               weights_y[j];
                    }
                    ans += val * weights_x[i];
                }
...
static bool spatial_transform(const uint8_t *image_data, int image_width,
                              int image_height, int image_channels,
                              uint8_t *crop_data, int crop_width,
                              int crop_height, const double *transformation,
    ...
    for (int n = 0; n < N; ++n) {
        ...
        for (int x = 0; x < dst_h; ++x)
            for (int y = 0; y < dst_w; ++y) {
                ...
                sampling(image_data, image_width, image_height, image_channels,
                         1.0 / scale, src_x, src_y, current_channel_data,
                         weights_x, weights_y, indices_x, indices_y, type);
            }
...
bool caculate_final_points(const float *points, int points_num,
                           const double *transformation, int pad_top,
                           int pad_left, float *final_points) {
...
#else
    if (t3t1_t0t4 < __FLT_EPSILON__ && t3t1_t0t4 > -__FLT_EPSILON__)
        t3t1_t0t4 = __FLT_EPSILON__ * 2;
#endif
    for (int i = 0; i < points_num; ++i) {
        float x = points[2 * i];
        float y = points[2 * i + 1];
        double fy =
            ((t[3] * x - t[0] * y) - (t[3] * t[2] - t[0] * t[5])) / t3t1_t0t4 +
            pad_top;
        double fx =
            ((t[1] * y - t[4] * x) - (t[1] * t[5] - t[4] * t[2])) / t3t1_t0t4 +
            pad_left;
...
bool face_crop_core(const uint8_t *image_data, int image_width,
                    ...
                    int mean_shape_width, int mean_shape_height, int pad_top,
                    int pad_bottom, int pad_left, int pad_right,
                    float *final_points, SAMPLING_TYPE type) {
    std::unique_ptr<double[]> transformation(new double[TFORM_SIZE]);
    bool check1 = transformation_maker(crop_width, crop_height, points,
                                       points_num, mean_shape, mean_shape_width,
                                       mean_shape_height, transformation.get());
    if (!check1) return false;
    bool check2 = spatial_transform(image_data, image_width, image_height,
                                    image_channels, crop_data, crop_width,
                                    crop_height, transformation.get(), pad_top,
                                    pad_bottom, pad_left, pad_right, type);
    if (!check2) return false;
    bool check3 = true;
    if (final_points) {
        check3 = caculate_final_points(points, points_num, transformation.get(),
